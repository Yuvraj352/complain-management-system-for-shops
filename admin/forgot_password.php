...
$query=mysqli_query($conn,"SELECT * FROM admin WHERE adminEmail='$email' and contactNo='$contact'");
$query1=mysqli_query($conn,"SELECT * FROM otp_expiry WHERE userEmail='$email' and otp='$ootp'");
$num=mysqli_fetch_array($query);
$num1=mysqli_fetch_array($query1);
if($num>0 && $num1>0)
{
mysqli_query($conn,"update admin set password='$password' WHERE adminEmail='$email' and contactNo='$contact'");
...
<input type="password" class="form-control unicase-form-control text-input" placeholder="Confirm Password" name="confirmpassword" required >
              ...
              <div class="registration">
		                <a href="http://localhost/cmp/admin/login.php">Sign in</a>
		            </div>
	  	</div>
    <!-- Optional JavaScript -->
