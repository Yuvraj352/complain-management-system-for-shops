...
$_SESSION['otp']=$_POST['otp'];
	$result = mysqli_query($conn,"select * from otp_expiry where otp='" . $_POST['otp'] . "' && is_expired!=1 && NOW() <= DATE_ADD(created_at, INTERVAL 10 MINUTE)");
	$num=mysqli_fetch_array($result);
	if($result->num_rows>0) {
		$result = mysqli_query($conn,"update otp_expiry set is_expired = 1 where otp = '" . $_POST['otp'] . "'");
		$success = 2;	
		header("location:http://localhost/cmp/user/forgot_password.php");
	} else {
		$success =1;
